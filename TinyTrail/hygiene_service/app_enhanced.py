...
BASELINE_DIR = REPO_ROOT / "vendor_baselines"
...
async def train_baseline(
    vendor_id: str = Form(...),
    baseline_images: List[UploadFile] = File(...)
) -> Dict[str, Any]:
    """
    ...
    if len(baseline_images) != 5:
        raise HTTPException(
            status_code=400,
            detail=f"Expected exactly 5 baseline images, received {len(baseline_images)}"
        )

    # Validate all files are images
    for i, file in enumerate(baseline_images):
...
async def verify_daily(
    ...
    daily_content_type = daily_image.content_type or ""
    daily_filename = daily_image.filename or ""
    if not daily_content_type.startswith('image/') and not daily_filename.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
        raise HTTPException(status_code=400, detail="Invalid image file")

    # Read daily image
    daily_image_bytes = await daily_image.read()
